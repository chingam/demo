...
d4.eo.stock.init = function() {
	d4.eo.stock.load("/stock/find/all");
	$(".modal-submit").on("click", d4.eo.stock.save);
	d4.eo.stock.search();
}

d4.eo.stock.add = function() {
	$("input[type=text], input[type=hidden], textarea").val("");
}

d4.eo.stock.save = function() {
	...
	$.ajax({
		...
		success : function(responseData) {
			if (responseData.status === "error") {
				d4.eo.stock.shoAlert(".modal-body", "modal", responseData.status, responseData.message);
				$("#loadingmask2").hide();
				return;
			}
			setTimeout(function() {
				d4.eo.stock.add();
				d4.eo.stock.load(url + "/find/all");
				$("#myModal").modal("hide");
				d4.eo.stock.shoAlert("#overview", "body", responseData.status,
...
d4.eo.stock.deletedata = function() {
		...
		$.ajax({
			...
			success : function(responseData) {
				setTimeout(function() {
					d4.eo.stock.load(url + "/find/all");
					$("#myModal").modal("hide");
					d4.eo.stock.shoAlert("#overview", "body", responseData.status, responseData.message);
				}, 1000);
			}
		});
...
d4.eo.stock.shoAlert = function(target, des, tag, message) {
	var cs, ic, posi;
...
d4.eo.stock.load = function(url) {
	...
	if (url != null) {
		...
		$("#datalistBody").load(url, function(responseTxt, statusTxt, xhr) {
			...
			$(".editBtn").bind("click", d4.eo.stock.edit);
			$(".deleteBtn").bind("click", d4.eo.stock.deletedata);
